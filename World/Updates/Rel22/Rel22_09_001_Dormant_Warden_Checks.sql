-- ----------------------------------------------------------------
-- Create the generic typed Warden catalogue ahead of the later server port.
-- This transition intentionally seeds no checks, addresses, module data,
-- requests, expected bytes, or other executable profiles.
-- ----------------------------------------------------------------
...
BEGIN
    ...
    SET @cNewContent = '001';
    SET @cNewDescription = 'Dormant_Warden_Checks';
    SET @cNewComment = 'Create an empty typed Warden catalogue for the later server port';
    ...
    IF (@cCurResult = @cOldResult) THEN
        ...
        -- At 22/08/001 this name is unowned. Removing residue from a failed
        -- CREATE keeps the non-transactional DDL safely re-runnable.
