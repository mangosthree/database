...
BEGIN
    ...
    SET @cNewDescription = 'Level_Command_Help';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'Help text for the new .level command, which sets a level where .levelup adds to it';
...
-- Help for the new `.level` command, which sets a character's level where
-- `.levelup` adds to it. Commands match by abbreviation, so `.level`
-- previously resolved to `.levelup` and there was nothing to document.
DELETE FROM `command` WHERE `command_text` = 'level';
INSERT INTO `command` (`id`, `command_text`, `security`, `help_text`) VALUES
(812, 'level', 3, 'Syntax: .level [$playername] #level\r\n\r\nSet the level of the character with $playername, or the selected player, to #level. Unlike .levelup this replaces the level instead of adding to it. Levels above 200 are refused: the client cannot render a spell tooltip for such a character.');
