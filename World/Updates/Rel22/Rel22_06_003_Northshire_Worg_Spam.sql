...
BEGIN
    ...
    SET @cNewDescription = 'Northshire_Worg_Spam';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'Slow infantry waypoint poll + widen worg buddy search to kill db_scripts log spam';
...
-- Northshire Stormwind Infantry poll their waypoint script every 500ms looking
-- for a Blackrock Battle Worg inside 8 yards. The worgs patrol wider than that,
-- so the search almost always misses and floods the log. Poll far less often and
-- widen the search to the distance the worgs actually roam.
UPDATE `creature_movement` SET `waittime`=25000 WHERE `script_id`=49869;

UPDATE `db_scripts` SET `search_radius`=25
    WHERE `script_type`=3 AND `id`=49869 AND `command`=26 AND `buddy_entry`=49871;
