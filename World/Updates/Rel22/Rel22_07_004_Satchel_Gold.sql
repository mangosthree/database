...
BEGIN
    ...
    SET @cNewDescription = 'Satchel_Gold';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'Money loot for the Call to Arms satchel, which the dungeon finder grants for filling a role the queue is short of';
...
-- Satchel of Exotic Mysteries: the Call to Arms reward. Its contents are
-- already in `item_loot_template` (64 rows), but the gold was never set, so
-- opening one paid nothing. The satchel always carried coin, reported across
-- the expansion at roughly 40 to 100 gold; the exact bounds were never
-- published, so 40g..99g is adopted. `Rate.Drop.Money` still applies.
UPDATE `item_template`
   SET `minMoneyLoot` = 400000, `maxMoneyLoot` = 990000
 WHERE `entry` = 69903;
