...
class WardenTransitionWorkflowTests(unittest.TestCase):
    ...
    def test_x64_catalogue_transition_preserves_x86_for_exact_rollback(self) -> None:
        ...
        self.assertNotRegex(
            sql,
            r"(?i)DELETE\s+FROM\s+`?warden_checks`?",
        )
        self.assertRegex(
            sql,
            r"COUNT\(\*\)\s+FROM\s+`warden_checks`\)\s*<>\s*252",
        )
    ...
    def run_update_failure(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            install = read_script("InstallDatabases.sh")
    ...
    def test_unix_dump_retains_prior_warden_files_when_no_tables_exist(self) -> None:
        script = read_script("Tools/dump_tables.sh")
        function = re.search(
            r"(?ms)^dump_warden_tables\(\)\s*\{.*?^\}\s*$", script
        )
